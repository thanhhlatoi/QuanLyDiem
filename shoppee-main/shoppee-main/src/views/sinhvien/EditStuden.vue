<template>
    <form @submit.prevent="editStuden()">
  ...
  export default {
    ...
    const getStuden = async()=>{
        try {
        const response = await axios.get(`sinhvien/${route.params.masinhvien}`); // Sử dụng makhoa từ route params
          studen.masinhvien = response.data.masinhvien,
          studen.hoten = response.data.hoten,
          studen.malop = response.data.lopHoc.malop
